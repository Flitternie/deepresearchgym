...
# Import functions from the original citation evaluation module
from eval_citation_async import (
...
async def evaluate_multi_answer_citation_query(semaphore, question_id, answer_paths, model):
    """
    Evaluate a single question with multiple answer files for citation quality and return averaged scores.
...
async def evaluate_multi_answer_citation_dataset(question_dir, answer_dir, model, num_workers=32):
    """
    Evaluate a dataset where questions and answers are in different directories,
    and each question may have multiple answer files for citation quality.
    ...
    for question_file in question_files:
        ...
        task = evaluate_multi_answer_citation_query(
...
def analyze_multi_answer_citation_results(results):
    """
    Analyze results from multi-answer citation evaluation and print statistics.
    ...
    print(f"Average normalized citation score across {count} questions: {average_score:.2f}")

    return {
        "average_citation_score": average_score,
...
async def main(args):
    """
    Main function to handle multi-answer citation evaluation.
    """
    print(f"Evaluating citation quality for questions from {args.question_dir} and answers from {args.answer_dir}")
    print(f"Using model: {args.model}")
    
    if args.times == 1:
        output_path = os.path.join(args.output, f"multi_answer_citation_{args.model}.json")
        
        results, avg_score = await evaluate_multi_answer_citation_dataset(
            args.question_dir, 
            args.answer_dir, 
            args.model, 
            args.workers
        ...
        analyze_multi_answer_citation_results(results)
        
    else:
        overall_results = []
        for i in range(args.times):
            print(f"\n=== Citation Evaluation Run {i+1}/{args.times} ===")
            output_path = os.path.join(args.output, f"multi_answer_citation_{args.model}_{i+1}.json")
            ...
            summary_results = analyze_multi_answer_citation_results(results)
            overall_results.append(summary_results)
        
        # Print overall results by averaging across all runs
        print("\n" + "="*60)
        print("OVERALL CITATION RESULTS ACROSS ALL RUNS:")
        print("="*60)
        
        avg_citation_score = sum(res["average_citation_score"] for res in overall_results) / len(overall_results)
        avg_answers_per_question = sum(res["avg_answers_per_question"] for res in overall_results) / len(overall_results)
        
        print(f"Average citation score across {args.times} runs: {avg_citation_score:.2f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Evaluate multiple answer files per question for citation quality")
